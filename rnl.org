...
*** 4.2.5 Generators
*** 4.2.6 Python Streams                                            :TOBEORG:
    /Streams/ offer another way to represent sequential data implicitly. 
    A ~stream~ is a lazily computed linked list. The ~rest~ of a ~Stream~ is itself a ~Stream~, which is computed lazily
    #+begin_src python :results output
      class Stream:
    ...
    - Example: increasing integers
      #+begin_src python :results output
        ...
        Stream(3, <...>)
      #+end_src
    - Example: ~map~ and ~filter~ on stream
      #+begin_src python :results output
        def map_stream(fn, s):
        ...
        def filter_stream(fn, s):
            ...
            if fn(s.first):
                ...
                return compute_rest()
      #+end_src
    - Example: using ~map_stream~ and ~filter_stream~ 
      #+begin_src python :results output
